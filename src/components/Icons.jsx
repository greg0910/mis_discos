export const SpotifyIcon = () => (
    <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor">
        <path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.241 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.6.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/>
    </svg>
)

export const AppleIcon = () => (
    <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor">
        <path d="M16.59 10.99c.029-2.736 2.228-4.043 2.327-4.103-1.266-1.854-3.231-2.108-3.929-2.138-1.666-.17-3.25.981-4.095.981-.845 0-2.148-.953-3.523-.925-1.79.027-3.444.981-4.364 2.587-1.87 3.243-.478 8.04 1.346 10.672.894 1.289 1.942 2.736 3.328 2.684 1.327-.055 1.831-.863 3.435-.863 1.594 0 2.057.863 3.445.836 1.417-.027 2.336-1.314 3.22-2.587 1.024-1.493 1.444-2.932 1.464-3.007-.033-.014-2.812-1.082-2.654-4.137zm-2.825-7.143c.731-.884 1.226-2.11 1.092-3.347-1.058.043-2.336.704-3.09 1.588-.673.784-1.266 2.036-1.103 3.246 1.18.092 2.368-.601 3.101-1.487z"/>
    </svg>
)

export const TidalIcon = () => (
    <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor">
        <path d="M12.005 8.243l-3.758 3.758 3.758 3.758 3.758-3.758-3.758-3.758zm-7.516 3.758l-3.758 3.758 3.758 3.758 3.758-3.758-3.758-3.758zm15.032 0l-3.758 3.758 3.758 3.758 3.758-3.758-3.758-3.758zm-7.516 7.516l-3.758 3.758 3.758 3.758 3.758-3.758-3.758-3.758zm0-15.032L8.247 8.243l3.758 3.758 3.758-3.758-3.758-3.758z"/>
    </svg>
)

export const SearchIcon = () => (
    <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
        <circle cx="11" cy="11" r="8"></circle>
        <line x1="21" y1="21" x2="16.65" y2="16.65"></line>
    </svg>
)
